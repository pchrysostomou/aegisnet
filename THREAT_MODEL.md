...
column holds only `test` and `accepted`. Three rows are `partial` today and they are the whole
remaining list, all three about how the deployment is built rather than what the code does:
read-only root filesystems and digest pinning (T-5.1), the lab's running pre-flight (T-5.5) and a
container image scan (T-5.6). Chunk 27 wrote the matrix and found eight; Chunk 28 closed the
upload deadline (T-1.4), the per-analyst and per-case brief limits (T-3.4) and the dashboard's
handling of characters that change what text says (T-4.4); Chunk 29 closed the lengthening
lockout (T-2.1) and the statement timeout (T-2.6).
