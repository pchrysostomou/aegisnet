...
- `domain/detectors/` — the detector contract (bounded windows and results, sampled
  evidence, dedup keys), the recorded severity formula, the registry and D-001 port scan
  (ADR-017, `docs/detection-rules.md`); pure like the rest of `domain/`
...
| `tests/detectors/` | `unit` | the detector contract's bounds, the severity formula and its reproduction, D-001 thresholds, guards, samples and purity, the registry, every labelled fixture under `tests/fixtures/labelled/` (pinned to `tools/gen_labelled_fixtures.py`), and the sweep service against fakes (registry sync, grid buckets, severity from the inventory, dedup on re-sweep, disabled and capped runs, a raising rule isolated) |
