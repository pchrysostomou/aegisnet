...
column holds only `test` and `accepted`. Five rows are `partial` today and they are the whole
remaining list. Chunk 27 wrote the matrix and found eight; Chunk 28 closed three of them — the
upload deadline (T-1.4), the per-analyst and per-case brief limits (T-3.4), and the dashboard's
handling of characters that change what text says (T-4.4).
