...
Status: **T1 and T2 results present (Milestone 2, Chunk 12); T3 absent.** §8 is written by `make eval` from
the labelled cases and the benign synthetic corpus, both authored in this repository, and is pinned by a test.
No lab traffic has been evaluated, so detector accuracy on real networks is **unmeasured** and AegisNet makes
no claim about it; the correlation and brief tables remain placeholders.
...
> events, and D-005 abstains there because the corpus carries no baselines. The lab corpus
> (T3) has not been run: AegisNet still makes **no** claim about accuracy on real networks.
> A test (`tests/detectors/test_evaluation.py`) pins this block to what the harness produces,
> so a rule change that moves a number must bring this document with it.
