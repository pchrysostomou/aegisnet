...
- Chunk 30 closed the last three, and they were the three about how the deployment is built.
  **T-5.1**: every service now runs on a read-only root filesystem. The writable paths were not
  ...
  no rootfs writes afterwards. **T-5.6**: an image scan exists, fails the job on an image this
  project builds, publishes a SARIF report for the two it only pulls, and ignores unfixed
  findings — a gate nobody can pass is a gate people learn to switch off. (This row first said
  the scan uploads no SARIF *because code scanning is not enabled on a private repository*. The
  repository was never private; see §4 R-12.)
