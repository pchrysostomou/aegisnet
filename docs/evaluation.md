...
Since Chunk 27 (ADR-034) the line carries all four, and one more: **the commit the corpus was last changed in**.
A content hash says *these are the right bytes*; a commit says *here is where to get them*, which is the question
a reader actually has. It is the last commit to touch `backend/tests/fixtures/labelled/` or the corpus — not
`HEAD`, which would go stale on the next unrelated push — and `tests/unit/test_provenance.py` asks git whether the
published sha is still that commit, skipping with a reason where git cannot answer (a shallow clone, an export).

Two consequences worth knowing before you run it. `make eval` needs a git checkout with history. And it refuses
while the corpus has uncommitted changes, because there is no honest commit for bytes that live nowhere — so
regenerating a corpus is `make gen-synthetic`, commit, then `make eval`, in that order.
...
Corpus commit `6ff7d3c798ed0ec39c16be52799d526451d6a243` · generator seed `20260905` · rule versions D-001 v1, D-002 v1, D-003 v1, D-004 v1, D-005 v1. The commit is the one that last changed the labelled cases or the corpus, so these bytes can be fetched rather than trusted.
...
```bash
make up && make migrate                    # the stack the suite drives, with its schema
make create-user EMAIL=analyst@example.test ROLE=analyst   # once; the password is read from stdin
