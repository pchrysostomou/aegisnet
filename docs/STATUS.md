...
| | |
|---|---|
| Phase | **M1 — Chunk 6 (auth, RBAC, audit, rate limits, HTTP routes) complete and verified locally; Chunk 7 (docs at the M1 gate with CI evidence) next** |
...
| SonarCloud | External GitHub App check (automatic analysis, not a workflow in this repository). **Quality gate failed** on every analysis so far. After Chunk 6 it failed two conditions; the Reliability one (two `S7493` bugs) and the Blocker behind *Security E* (upload bytes → spool name → path) were found with a local SonarQube Community scan and fixed (E-39, E-40). One condition remains, *Security Rating on New Code C*, present since E-17 and readable only on the private dashboard |
...
| M1 Foundation / ingest / normalize / assets | 🟡 In progress — Chunks 1–6 done | E-1 – E-8, E-18 – E-38 | Next: Chunk 7, the documents at the M1 gate |
...
1. SonarCloud: the gate now fails on one condition, *Security Rating on New Code C* (E-40), a Major finding that only the private dashboard can name. Read it there (Issues → Type: Vulnerability; also the Security Hotspots tab) and fix it, or remove the GitHub App from the repository; the local Community scan cannot see taint findings and three blind fixes (E-39, E-40) did not clear it.
2. Chunk 5 is confirmed in CI (E-33); nothing is outstanding for it.
3. Chunk 6 is confirmed in CI (E-38); nothing is outstanding for it.
4. Chunk 7: bring the documents to the Milestone 1 gate — README demo script reproducible from a clean clone, `docs/evaluation.md` stating that detector accuracy is unmeasured, the DoD checklist reconciled with CI evidence, and the remaining API acceptance box (audited traversal attempts) either closed or explicitly deferred.
5. Keep this file and `THREAT_MODEL.md` updated per chunk, not afterwards.
