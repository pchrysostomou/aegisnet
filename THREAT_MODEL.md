...
| T-1.4 | DoS | Decompression bomb / 10 GB NDJSON / 10M-line batch | Hard caps: request body size, max lines per batch, max line length, streaming line-by-line parse (never `json.load` whole file), upload timeout | Per-line caps: `backend/tests/security/test_payload_limits.py` (byte cap, multibyte, 20 000-level nesting refused by the scanner, key/item counts) — Chunk 3; line-count cap: `tests/unit/test_ingest_service.py` (batch marked failed, valid events kept) — Chunk 4; body and timeout caps arrive with the HTTP route in Chunk 6 |
