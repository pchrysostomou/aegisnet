...
| Time model | All timestamps stored UTC `timestamptz`; UI renders in Europe/London by default | Avoids the classic correlation-off-by-an-hour class of bug |
...
- [ ] `docker compose up --build` starts api, worker, scheduler, postgres, redis, web — all healthchecked.
- [ ] A documented safe sample dataset ingests via one README command.
- [ ] All five detectors have labelled **positive and negative** fixtures; precision/recall reported in `docs/evaluation.md`.
- [ ] Unit + integration tests pass in GitHub Actions on push and PR.
- [ ] A reviewer reproduces the main demo from the README with no undocumented steps.
- [ ] `README.md`, `ARCHITECTURE.md`, `THREAT_MODEL.md`, `SECURITY.md`, `docs/STATUS.md`, `docs/detection-rules.md`, `docs/evaluation.md`, `docs/perplexity-integration.md` all current.
- [ ] Screenshots + a ≤3-minute demo script exist.
- [ ] Auth, RBAC, audit logging, rate limiting implemented and tested.
- [ ] Redaction test suite proves no forbidden field class can reach the Perplexity request body.
- [ ] `docs/RELEASE_CHECKLIST.md` completed and signed off before tagging `v1.0.0`.
