...
| T-4.2 | Tampering | Hallucinated CVEs, threat actors, or IOCs presented as fact | An external claim requires a citation, and a citation must be an https URL with no credentials in its authority which the brief itself lists — a claim citing an id the brief does not carry is a refusal. **The URL is never fetched** (R-7): this checks the shape of a source and that it was declared, not that it exists or says what the claim says. Uncited claims stored and rendered **UNVERIFIED**; UI visually separates "observed facts (from your data)" from "external research claims" | Schema + citation-enforcement tests with a fixture response containing uncited claims |
...
| T-5.4 | Info disclosure | Secrets committed | `.env` gitignored, `.env.example` only, pre-commit + CI secret scanning, no secrets in compose defaults | `backend/tests/security/test_env_template.py` (every secret variable is a placeholder) and the pre-commit hook — Chunk 1; the `security` workflow's gitleaks job, **at two scopes**: the commits of a push, and the **entire history** on the weekly run or a manual dispatch (E-38, E-104). This cell used to say "history and diff on every push", which was never true and hid a job that was green on every push and red every Monday — seven fake credentials committed as literals on 2026-09-05 and rewritten to runtime expressions, which clears the narrow scan and cannot clear the wide one. `.gitleaks.toml` allows those four strings **by anchored value, never by path**, so a real secret in a test file is still a finding; `tests/security/test_secret_scan_config.py` is what holds the allowlist to that ([ADR-038](docs/adr/ADR-038-the-security-tab-said-two-things-and-both-were-wrong.md)) |
...
| T-5.6 | Tampering | Vulnerable dependency | Pinned lockfiles, Dependabot alerts, `pip-audit` + `pnpm audit`, **and a container image scan**, because a lockfile cannot see a base image: a CVE in the distribution's openssl ships in every container and appears in neither audit | The `security` workflow: `pip-audit --strict` on the exported lockfile and `pnpm audit --prod` on every push (E-12, E-38); Dependabot alerts enabled, which is a repository setting rather than something a run can evidence — it is recorded in the Repository row of `docs/STATUS.md` and carries no evidence id; and from Chunk 30 the `images` job, which builds what the stack builds and scans it with Trivy alongside the two images the stack pulls. It **fails the job** on an image this project builds *and* **publishes SARIF** for that same image, so a finding anybody here can close both blocks the push and lands in the Security tab. The two images the stack only pulls are scanned, printed in full and weekly, and file no alerts (R-10). This cell has been wrong twice: it first said the scan uploads no SARIF *because code scanning is not enabled on a private repository* — the repository has been public since it was created (R-12) — and Chunk 33's correction then published the **pulled** images, which put 74 permanently open HIGH and CRITICAL alerts on this project for Go standard-library CVEs inside `postgres:16-alpine`'s `gosu` binary, fixed in Go 1.24.13 and shipped by nobody here. The reasoning survived and the target moved ([ADR-038](docs/adr/ADR-038-the-security-tab-said-two-things-and-both-were-wrong.md)). `tests/security/test_image_scan.py` fails if a service is added whose image nothing scans, and if a built image is ever allowed to report instead of gate |
...
| R-4 | **No secrets manager**; secrets sit in `.env` on the host. | Self-hosted single-node scope; a KMS would add deployment burden with little benefit at this scale. | `make bootstrap` creates `.env` through `os.open(…, 0o600)` itself, so there is no moment when it is readable by anyone else (`infra/scripts/bootstrap_env.py`, documented under *Secrets* in `README.md`); `.env` and `.env.*` are gitignored with only `.env.example` excepted, and a pre-commit hook refuses them; the `security` workflow's gitleaks job scans the push's own commits on a push and the whole history weekly or on demand (E-104). **Rotation is not covered.** No procedure for changing `SECRET_KEY` or a database password is written down anywhere, and this row used to claim one was |
...
| R-7 | **No independent verification of citation *content*** — and none of its existence either. The check is that a citation is an https URL with no credentials in its authority and that the brief lists the id a claim cites; nothing ever fetches the URL, so the model saying a page is there is the only evidence that it is. | Full fact-checking is out of scope. Fetching would also open a second egress path, to an address a model chose, when TB-3 is deliberately one pinned host — the wrong trade for what it buys. | Every claim is marked verified or unverified in the dashboard and in the exported report, and an uncited external claim is kept and marked `UNVERIFIED` rather than dropped; a URL the browser would not follow is printed as text with a line saying why (T-4.4) |
| R-10 | **Base images are pinned by tag, not by digest, so a moved tag changes what ships.** | Digest pinning buys reproducibility and resistance to a tampered upstream tag. It also freezes the image: at the time nothing in this repository bumped a digest — there was no `dependabot.yml` at all — so pinning without an updater would stop security patches arriving, on a single-maintainer project, which is the larger risk here. Decision F-5 chose tags; Chunk 30 re-examined it and kept it, with the reasoning written down rather than deferred. **Half of that reasoning has since expired**: Chunk 32 added `.github/dependabot.yml` with a `docker` ecosystem, so the updater the argument said did not exist now does. The decision stands until somebody revisits it deliberately, which is [#14](https://github.com/pchrysostomou/aegisnet/issues/14). | The image scan (T-5.6) reads what is actually inside every image on every push and weekly. It **gates** on the two images this project builds, where a finding is actionable — and earned that on its first run, catching npm's bundled `tar` and an openssl release behind, both fixed rather than suppressed. It **reports without gating** on `postgres:16-alpine` and `redis:7-alpine`, because the only responses to a finding there are to wait for the publisher or to stop using PostgreSQL: one architecture showed twenty-seven HIGHs in util-linux and the Go standard library and another showed a different set, none reachable from anything here. Two things this does not cover, said plainly: a tag that moves to something malicious but free of known CVEs, which is what digest pinning would have caught; and a real future vulnerability in the datastore images, which will be printed and will not stop a push. The lab's Suricata sensor stays pinned by digest — third-party, pointed at hostile traffic, no reason to float |
...
| R-12 | **The repository is public, so every detection threshold, guard and hard negative is readable by anyone — including someone shaping traffic to stay under them.** `docs/detection-rules.md` publishes the default parameters of all five rules, and this file publishes the model itself. | Deliberate: this is a portfolio and learning project whose whole claim is that its reasoning can be checked, and a detector whose security depends on its thresholds being secret is not a detector anybody should trust. Deterministic heuristics are evadable by an informed attacker whether or not the numbers are published (R-1 already says so); publishing them buys review, and hiding them would buy the appearance of strength. | Parameters are settings rather than constants, so a deployment can move off the published defaults without a fork; alerts carry the rule version and the evidence they fired on, so evasion shows up as a rule that stopped firing rather than as silence. **This row was added on 2026-09-07**, when a check found the repository had been public since creation while the project's documents had assumed for thirty-two chunks that it was private — see the correction in [ADR-037](docs/adr/ADR-037-the-last-three-rows-are-about-the-deployment.md). Nothing sensitive was exposed: no `.env`, key, capture or log is tracked, which `make verify-ignore` and the `gitleaks` job both assert — and the history scan that ran on 2026-09-07 says so over all 187 commits, not just the recent ones (E-104) |

## 5. Review cadence

Re-run this model at each milestone gate. Any new external egress, new endpoint, or new rendered field requires
a new row here before merge. [`docs/RELEASE_CHECKLIST.md`](docs/RELEASE_CHECKLIST.md) held `v1.0.0` until every
mitigation above had a named passing test or an explicit accepted-risk entry, and holds every tag after it to the
same rule — **§6 is where that is written down and checked**, and it is the artefact to read if you want to know
what this project has actually proven rather than what it intends.

### Gate reviews, in the order they happened

The heading here used to say *Milestone 1 gate review (2026-09-05)*, because that is what it was
when it was written. It has since collected every review from Chunk 16 to Chunk 30, so it is
renamed rather than left describing a fraction of itself. The bullets stay in sequence and are not
rewritten: a count of unfinished rows in one of them is true **as at that chunk** and nowhere else.
§6 is the only current answer to that question, and it is the one a test checks.
...
- Chunk 30 closed the last three, and they were the three about how the deployment is built.
  **T-5.1**: every service now runs on a read-only root filesystem. The writable paths were not
  ...
  no rootfs writes afterwards. **T-5.6**: an image scan exists, gates *and* reports on an image
  this project builds, scans and prints the two it only pulls without filing alerts against this
  project, and ignores unfixed findings — a gate nobody can pass is a gate people learn to switch
  off. (This row has been wrong twice. It first said the scan uploads no SARIF *because code
  scanning is not enabled on a private repository*; the repository was never private, see §4 R-12.
  The correction then published the pulled images, and 74 alerts nobody here could close arrived
  on a public repository — the alert-shaped form of the same mistake. ADR-038.)
...
seven `backend/tests/load/` tests are opt-in and run under `make load-test` against a deployment
...
| T-5.4 | test | `backend/tests/security/test_env_template.py::test_every_secret_variable_is_a_placeholder`, `backend/tests/security/test_env_template.py::test_gitignore_blocks_secrets_and_captures`, `backend/tests/security/test_env_template.py::test_pre_commit_refuses_dotenv_and_captures`, `backend/tests/security/test_compose_policy.py::test_no_inline_secret_literals`, `backend/tests/security/test_secret_scan_config.py::test_every_default_rule_is_still_on`, `backend/tests/security/test_secret_scan_config.py::test_nothing_is_exempted_by_location`, `backend/tests/security/test_secret_scan_config.py::test_the_allowed_values_are_matched_whole_and_not_as_prefixes`, `backend/tests/security/test_secret_scan_config.py::test_no_allowed_value_appears_as_a_literal_in_the_tree`, `backend/tests/security/test_secret_scan_config.py::test_both_readers_of_this_config_are_pointed_at_it`, `.github/workflows/security.yml::secrets` | R-4 |
...
| T-5.6 | test | `.github/workflows/security.yml::python-deps`, `.github/workflows/security.yml::node-deps`, `.github/workflows/security.yml::images`, `backend/tests/security/test_runtime_dependencies.py::test_every_third_party_import_in_src_is_a_runtime_dependency`, `backend/tests/security/test_image_scan.py::test_the_image_scan_covers_every_image_the_stack_runs`, `backend/tests/security/test_image_scan.py::test_a_finding_in_an_image_this_project_builds_fails_the_job`, `backend/tests/security/test_image_scan.py::test_an_image_this_project_only_pulls_is_reported_and_not_gated_on`, `backend/tests/security/test_image_scan.py::test_nothing_is_suppressed_by_an_ignore_file`, `backend/tests/security/test_image_scan.py::test_the_images_this_project_builds_publish_where_a_finding_can_be_read`, `backend/tests/security/test_image_scan.py::test_publishing_survives_the_failure_it_is_describing`, `backend/tests/security/test_image_scan.py::test_the_job_may_publish_and_the_lockfile_audits_may_not`, `backend/tests/security/test_dockerfiles.py::test_the_dashboard_runtime_ships_no_package_manager`, `backend/tests/security/test_image_scan.py::test_unfixed_findings_do_not_turn_every_push_red` | R-10 |
<!-- coverage:end -->
