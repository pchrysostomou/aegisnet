...
| SonarCloud | External GitHub App check (automatic analysis, not a workflow in this repository). **Quality gate failed** on every analysis so far; after Chunk 6 two conditions fail, *Security Rating on New Code E* and *Reliability Rating on New Code C*. The findings are not readable from the check or the public API (private project). A local SonarQube Community scan of the same tree (E-39) reproduced the Reliability C exactly (two `S7493` bugs, fixed) and reports no vulnerability, so the Security E comes from SonarCloud's taint analysis, which Community lacks; the likeliest flow (upload bytes → spool entry name → file path) is removed in E-39 and confirmation needs the next analysis or the dashboard |
...
1. SonarCloud: read the check on the commit carrying E-39. The Reliability condition should now pass; if *Security Rating on New Code* is still below A, the remaining finding has to be read on sonarcloud.io (Issues → Type: Vulnerability, or the Security Hotspots tab) or the GitHub App removed from the repository, because the local Community scan cannot see taint findings.
